"""
FastAPI streaming routes (thumbnails, subtitles, media).

Exposes the /stream family of endpoints used by Stremio and WebDAV.
Handles token verification, client selection (load + DC aware), single-file
streaming, multi-part virtual streaming, and ZIP-entry streaming. Also
provides decay_client_failures and the shared _streamer_by_client cache.
"""
...
def select_best_client(target_dc: int) -> int:
    ...
    if target_dc > 0:
        matching = [
            idx for idx, dc in client_dc_map.items() if dc == target_dc and idx in multi_clients
        ]
    else:
        matching = []
    if not matching:
...
def _resolve_filename_mime(file_id):
    ...
    mime_type = (
        file_id.mime_type or mimetypes.guess_type(file_name)[0] or "application/octet-stream"
    )
...
def _content_disposition(file_name, disposition="inline"):
    ascii_fallback = (
        file_name.encode("ascii", "ignore").decode("ascii").replace('"', "").strip() or "file"
    )
...
async def subtitle_handler(
    token: str, id: str, name: str, token_data: dict = Depends(verify_token)
):
    try:
        decoded = await decode_string(id)
        chat_id = int(f"-100{decoded['chat_id']}")
...
async def stream_handler(
    request: Request, token: str, id: str, name: str, token_data: dict = Depends(verify_token)
):
...
async def media_streamer(
    ...
    if parallelism > 1 and len(multi_clients) > 1:
        other_indices = sorted(
            (i for i in multi_clients if i != index), key=lambda i: work_loads.get(i, 0)
        )
    ...
    headers, status = _build_stream_headers(
        mime_type, file_name, req_length, range_header, start, end, file_size
    )
...
async def virtual_media_streamer(
    ...
    common_headers, status = _build_stream_headers(
        mime_type, file_name, req_length, range_header, start, end, file_size
    )
    ...
    return StreamingResponse(
        body_gen, headers=common_headers, status_code=status, media_type=mime_type
    )
...
async def global_media_streamer(
    request: Request,
    chat_id: int,
    msg_id: int,
    token: str,
    ...
    if streamer is None:
        raise HTTPException(
            status_code=503, detail="Global Search streaming is unavailable (no Userbot configured)"
        )
...
async def global_virtual_media_streamer(
    request: Request,
    parts_payload: list,
    token: str,
    token_data: dict = None,
    ...
    if streamer is None:
        raise HTTPException(
            status_code=503, detail="Global Search streaming is unavailable (no Userbot connected)"
        )

    parts, file_size = await resolve_virtual_parts(parts_payload, streamer, prefix_100=False)
    if not parts or file_size <= 0:
        raise HTTPException(
            status_code=404, detail="Split media parts not accessible via Global Search"
        )
...
async def _zip_media_streamer(
    ...
    if streamer is None:
        raise HTTPException(
            status_code=503, detail="ZIP streaming is unavailable (no client/session)"
        )
    ...
    async def _read(off, length):
        return await _read_virtual_range(
            parts, off, length, streamer, request, client_index, parallelism, prefetch_count
        )
    ...
    stream_id = secrets.token_hex(8)
    inner_name = (
        (entry.get("name") or "").split("/")[-1]
        or unquote(request.path_params.get("name", ""))
        or "video.mkv"
    )
    ...
    headers, status = _build_stream_headers(
        mime_type, inner_name, req_length, range_header, start, end, inner_size
    )
    if request.method == "HEAD":
        return PlainResponse(status_code=status, headers=headers)
...
async def global_zip_media_streamer(
    request: Request,
    parts_payload: list,
    token: str,
    token_data: dict = None,
...
async def db_zip_media_streamer(
    request: Request,
    parts_payload: list,
    token: str,
    token_data: dict = None,
