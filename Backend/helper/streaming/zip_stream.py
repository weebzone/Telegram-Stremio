"""
On-the-fly ZIP entry extraction for streaming.

Parses local file headers / central directory (including Zip64) of a ZIP
that itself lives on Telegram, locates a named entry, and returns the
compressed/uncompressed offset + size so the caller can stream only that
entry without downloading the whole archive.
"""
...
def _parse_central_directory(tail, tail_base, zip_size):
    ...
    if uncomp == 0xFFFFFFFF or comp == 0xFFFFFFFF or local_offset == 0xFFFFFFFF:
        uncomp, comp, local_offset = _zip64_sizes(
            extra, uncomp, comp, need_offset=True, offset=local_offset
        )
    return {
        "method": method,
        "name": name,
