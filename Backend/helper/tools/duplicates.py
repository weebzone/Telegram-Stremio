"""
tools/duplicates.py — find and purge duplicate stream entries.

Scans the library for the same file appearing more than once (by size /
filename / hash heuristics) and exposes start / status / purge for the
Tools-page Duplicates panel.
...
class DuplicateManager:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if s.get("purge_started_at"):
            p_end = s.get("purge_finished_at") or now()
        ...
        return {
            ...
            "purge_elapsed": fmt_elapsed(p_elapsed),
            "purge_eta": fmt_elapsed(p_eta) if p_eta else "—",
        }
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            s["finished_at"] = now()
            LOGGER.info(
                f"[Duplicates] {s['status']} — {len(s['groups'])} group(s), {s['duplicate_count']} redundant"
            )
    ...
    async def purge(
        self, stream_ids: Optional[List[str]] = None, delete_all: bool = False
    ) -> Dict[str, Any]:
        async with self._lock:
            if self.state.get("purge_status") == "running":
