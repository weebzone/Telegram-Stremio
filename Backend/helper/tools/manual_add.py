"""
tools/manual_add.py — manual media add and caption stamping helpers.

Used by the Tools-page Manual Add flow and by the bot receiver when a user
points at a Telegram post.  Parses t.me links, resolves the message, and
stamps ID captions onto media.

Example
-------
    from Backend.helper.tools.manual_add import (
        parse_telegram_link, resolve_telegram_message, stamp_caption_by_ref
    )

    chat, msg_id = parse_telegram_link("https://t.me/c/1234567890/42")
    info = await resolve_telegram_message(client, url="https://t.me/...")
    await stamp_caption_by_ref(client, chat_id, msg_id, media_doc)
"""
...
async def resolve_telegram_message(client, url: str = None, chat_id=None, msg_id=None) -> dict:
    if url:
        ...
        if chat_ref is None:
            raise ValueError(
                "Could not read that Telegram link. Use a t.me/c/... or t.me/<channel>/... message link."
            )
...
async def stamp_caption_with_id(message, metadata_info: dict) -> bool:
    try:
        ...
        LOGGER.warning(
            f"[Caption] Could not stamp id on message {getattr(message, 'id', '?')}: {e}"
        )
        return False
