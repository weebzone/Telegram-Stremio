"""
tools/dbcheck.py — dead / missing Telegram message checker.

Walks library stream entries and verifies the underlying Telegram messages
still exist.  Results feed the Tools-page DB Check UI (list + purge).
...
-------
    ...
    status = dbcheck_manager.get_status()
...
class DbCheckManager:
    ...
    async def _record_results(self, batch: List[str], results) -> None:
        ...
        for stream_hash, result in zip(batch, results):
            ...
            if result is True:
                ...
                s["errors"] += 1
        elapsed = max(1, int(now() - s["started_at"]))
            ...
            for i in range(1, db.current_db_index + 1):
                ...
                while not self._cancel:
                    ...
                    docs = (
                        ...
                        .find(query)
                        .sort("_id", 1)
                        .limit(DBCHECK_PAGE_SIZE)
                        .to_list(length=DBCHECK_PAGE_SIZE)
                    )
                    if not docs:
                        break
                    for movie in docs:
    ...
    async def purge(self, stream_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        return {
            "ok": True,
            "message": f"Purged {purged} dead entr{'y' if purged == 1 else 'ies'}.",
            "purged": purged,
        }
