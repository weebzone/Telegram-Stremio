"""
tools/scan.py — channel scan / rescan job manager.

Drives the Tools-page Scan and Rescan actions: walks configured Telegram
channels, processes media messages, and persists progress so a job can be
resumed or cancelled from the admin UI.
...
-------
    ...
    result = await scan_manager.start(client, channels=["-100123"], mode="scan")
...
class ScanManager:
    ...
    async def load(self, db) -> None:
        self._db = db
        try:
            doc = await db.dbs["tracking"][STATE_COLLECTION].find_one({"_id": SCAN_DOC_ID})
        ...
        if doc:
            ...
            if self.state["status"] == "paused":
                LOGGER.info(
                    "[ScanManager] Found an interrupted scan — marked as paused (resumable)."
                )
            await self._persist()
        else:
    ...
    async def _persist(self) -> None:
        ...
        try:
            doc = dict(self.state)
            doc["_id"] = SCAN_DOC_ID
            await self._db.dbs["tracking"][STATE_COLLECTION].update_one(
                {"_id": SCAN_DOC_ID}, {"$set": doc}, upsert=True
            )
        except Exception as e:
    ...
    async def start(self, client, channels: List[str], mode: str = "scan") -> Dict[str, Any]:
        async with self._lock:
            ...
            return {
                "ok": True,
                "message": f"{'Rescan' if mode == 'rescan' else 'Scan'} started.",
    ...
    async def cancel(self) -> Dict[str, Any]:
        ...
        return {
            "ok": True,
            "message": "Stop requested — the scan will pause after the current batch.",
        }
    ...
    async def _scan_channel(self, client, chat_id: int, ch_key: str) -> bool:
        ...
        LOGGER.info(
            ...
            + (
                f" up to {last_id} (probe)"
                if use_probe
                else " (heuristic mode — probe unavailable)"
            )
        )
